...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.data_source.TUSHARE_TOKEN': {
    title: 'Tushare Token',
    summary: '用于访问 Tushare Pro 数据接口。支持官方 Token 和 API 代理中转 Token。',
    usage: '官方模式：在 Tushare 账户中获取 token 后直接填入。\n代理模式：从 API 中转站获取 Token 后填入，并在下方 TUSHARE_PROXY_URL 中配置代理地址。',
    valueNotes: ['不同 Tushare 权限会影响可用接口和数据完整度。', '使用 API 代理中转 Token 时，必须同时配置 TUSHARE_PROXY_URL。'],
    impact: ['影响部分 A 股基础数据、股票列表和相关增强数据获取。'],
    notes: ['不要把 token 提交到仓库或公开日志。', '代理模式下系统会在启动时验证代理端点可达性。'],
  },
  'settings.data_source.TUSHARE_PROXY_URL': {
    title: 'Tushare 代理地址',
    summary: 'Tushare API 代理服务器地址。配置后所有 Tushare 请求将通过此代理转发。',
    usage: '从 API 中转服务商获取代理地址后填入，例如 http://your-proxy-server.com/tushare。\n不填写则使用 Tushare 官方 API（api.tushare.pro）。',
    valueNotes: ['仅在使用 API 代理中转 Token 时需要配置。', '代理地址需包含完整 URL（含协议和路径）。', '系统启动时会自动验证代理端点可达性。'],
    impact: ['影响所有 Tushare API 请求的目标地址。'],
    notes: ['请确保代理服务器稳定可用，不可达时该数据源将不可用。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.data_source.TUSHARE_TOKEN': {
    title: 'Tushare Token',
    summary: 'Token used for Tushare Pro data access. Supports both official Tushare tokens and API proxy relay tokens.',
    usage: 'Official mode: paste the token from your Tushare account.\nProxy mode: paste the relay token and configure the proxy URL in TUSHARE_PROXY_URL below.',
    valueNotes: ['Available APIs depend on your Tushare permission level.', 'When using a proxy relay token, TUSHARE_PROXY_URL must also be configured.'],
    impact: ['Affects some A-share base data, stock lists, and enrichment data.'],
    notes: ['Do not commit the token or print it in public logs.', 'In proxy mode, the system validates proxy endpoint reachability at startup.'],
  },
  'settings.data_source.TUSHARE_PROXY_URL': {
    title: 'Tushare Proxy URL',
    summary: 'Tushare API proxy server address. When configured, all Tushare requests are routed through this proxy.',
    usage: 'Enter the proxy URL from your API relay provider, e.g. http://your-proxy-server.com/tushare.\nLeave empty to use the official Tushare API (api.tushare.pro).',
    valueNotes: ['Only required when using an API proxy relay token.', 'Must include the full URL with protocol and path.', 'The system validates proxy endpoint reachability at startup.'],
    impact: ['Affects the target address for all Tushare API requests.'],
    notes: ['Ensure the proxy server is stable and reachable; otherwise this data source will be unavailable.'],
  },
  'settings.data_source.TICKFLOW_API_KEY': {
    title: 'TickFlow API Key',
